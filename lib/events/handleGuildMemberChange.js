...
exports.handleJoin = (member, client) => {
  console.log(member)
  if (member.guild.channels.exists('name', 'general')) member.guild.channels.find('name', 'general').send(userJoin(member))
}

exports.handleLeft = (member) => {
  if (member.guild.channels.exists('name', 'general')) {
    member.guild.channels.find('name', 'general').send(userLeft(member))
  }
}
